...
def benchmark(SEQ_LEN, mode, provider, device=DEVICE):
    ...
    if provider == "triton":
        fn = lambda: triton_attention_v1(q, k, v, sm_scale)
    ms = triton.testing.do_bench(fn)
    total_flops = 4.0 * BATCH * N_HEADS * SEQ_LEN * SEQ_LEN * HEAD_DIM # * 0.5 (causal=False)
...
def test_flashattention_kernel(B, H, N, d, device=DEVICE, atol=5e-3):
    ...
    scale = 1 / math.sqrt(d)
    tri_attn = triton_attention_v1(q, k, v, scale)
    torch_attn = torch.nn.functional.scaled_dot_product_attention(q, k, v, is_causal=False)
